...
blit :: Tex -> Int'' -> Int'' -> Float'' -> M ()
blit tex x y r = fun3 (blit' tex) x y r
...
load_tex fn = do
  let sz = maybe (error $ "unknown texture:" ++ show fn) id $ lookup fn textures
...
lookupChar (x, ys) c = do
  i <- E.cast c - 32
  t <- load =<< ix x i
  (w:h:bs) <- mapM (\y -> load =<< ix y i) ys
  return ((t,(w,h)), fmap return bs)
  
textures = [ a | (_,_,_,_,a) <- concatMap snd metrics ]
